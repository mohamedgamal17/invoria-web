...
export type ProfitMetricsFilters = {
  period: ReportPeriod;
};

@Component({
  selector: 'app-order-profit-metrics-filter-panel',
  standalone: true,
  imports: [CommonModule, FormsModule, ButtonModule, SelectModule, SurfaceCardComponent],
  templateUrl: './order-profit-metrics-filter-panel.component.html'
})
export class OrderProfitMetricsFilterPanelComponent {
  period = input<ReportPeriod>(ReportPeriod.Daily);
  loading = input(false);

  filtersChange = output<ProfitMetricsFilters>();
