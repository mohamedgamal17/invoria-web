...
@Component({
  selector: 'app-order-sales-metrics-list',
  standalone: true,
  imports: [CommonModule, TableModule, ButtonModule, PaginatorModule, SkeletonModule, EmptyStateComponent, SurfaceCardComponent],
  templateUrl: './order-sales-metrics-list.component.html'
})
export class OrderSalesMetricsListComponent {
  readonly ReportPeriod = ReportPeriod;

  rows = input<OrderSalesReportPeriod[]>([]);
