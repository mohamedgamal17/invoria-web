...
export type PurchaseCompletionMetricsFilters = {
  period: ReportPeriod;
};

@Component({
  selector: 'app-purchase-completion-metrics-filter-panel',
  standalone: true,
  imports: [CommonModule, FormsModule, ButtonModule, SelectModule, SurfaceCardComponent],
  templateUrl: './purchase-completion-metrics-filter-panel.component.html'
})
export class PurchaseCompletionMetricsFilterPanelComponent {
  period = input<ReportPeriod>(ReportPeriod.Daily);
  loading = input(false);

  filtersChange = output<PurchaseCompletionMetricsFilters>();
